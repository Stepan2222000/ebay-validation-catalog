...
CFG_KW = dict(checks=['dedup', 'condition', 'blocklist', 'whitelist', 'price'],
              blocklist=[{'pattern': 'fake', 'match_type': 'word'}])


class BoomVD:
    """Прокси соединения валидатора, у которого падает первая же запись.

    Сбой входит через настоящую границу модуля (контракт соединения),
    а не через подмену внутренних ссылок events.
    """
    ...
    async def executemany(self, *a, **k):
        raise RuntimeError('искусственное падение записи в БД валидатора')


async def tick_state(vd):
    return {r['item_id']: (r['status'], list(r['reject_reasons']))
            for r in await vd.fetch('select * from validated_items')}


async def main():
    cfg = write_cfg(**CFG_KW)
